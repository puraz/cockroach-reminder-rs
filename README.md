...
| 技术 | 选型 |
|---|---|
| GUI / 渲染 | **iced 0.13**（`canvas` + `wgpu`） |
| 动画 | iced 订阅（16ms 动画时钟） |
| 托盘菜单 | **tray-icon** |
| 透明/点击穿透/置顶覆盖层 | iced 多窗口 + **objc2** 调用 AppKit（`setIgnoresMouseEvents` / `setLevel` / `collectionBehavior`） |
| 多显示器 | `NSScreen.screens`（objc2） |
| 设置持久化 | **serde_json**（`~/Library/Application Support/com.cockroach.reminder/config.json`） |
| 系统通知 | **notify-rust** |
